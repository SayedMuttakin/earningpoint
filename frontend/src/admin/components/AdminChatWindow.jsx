            ...
            return (
              <div key={i} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex gap-2 max-w-[80%] ${isAdmin ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-8 h-8 ${isAdmin ? 'rounded-lg border border-indigo-200 shadow-xs' : 'rounded-full bg-slate-700'} flex-shrink-0 flex items-center justify-center overflow-hidden`}>
                    {isAdmin ? <img src="/zenivio-logo.png" alt="Zenivio Support" className="w-full h-full object-cover" /> : <User className="w-4 h-4 text-slate-300" />}
                  </div>
