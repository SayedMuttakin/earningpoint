...
const PostCard = ({ post }) => {
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-1.5">
            ...
            {post.isVerified !== false && (
              <BadgeCheck 
                ...
                className="w-[18px] h-[18px] fill-blue-500 text-white flex-shrink-0 cursor-pointer" 
              />
            )}
          </div>
      ...
      {showVerifiedPopup && createPortal(
        <AnimatePresence>
          {showVerifiedPopup && (
             <div className="fixed inset-0 z-[9999] flex items-end justify-center sm:items-center p-0 sm:p-4">
              ...
              <motion.div 
                ...
                className="relative w-full max-w-sm bg-white rounded-t-3xl sm:rounded-3xl p-6 sm:p-8 shadow-2xl flex flex-col items-center z-10 mx-auto"
              >
                <div className="w-10 h-1 rounded-full bg-slate-200 mb-6 sm:hidden" />
                <BadgeCheck className="w-10 h-10 fill-blue-500 text-white mb-4" />
                <h3 className="text-xl font-bold text-slate-900 text-center mb-3">Account information</h3>
                <p className="text-[#536471] text-[15px] text-center mb-8 leading-relaxed">
                ...
                <button 
                  onClick={() => setShowVerifiedPopup(false)}
                  className="w-full py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-900 font-bold text-[15px] rounded-full transition-colors active:scale-[0.98]"
                >
                  Cancel
                </button>
              </motion.div>
