...
for line in lines:
    if 'animate={{' in line and (('repeat: Infinity' in line) or ('repeat:Infinity' in line)):
        ...
        # Remove animate and transition props by taking the whole word animate={{...}} matching balanced braces approximately
        # Since we just want to remove them, we can use a non-greedy match up to the end of the prop
